//
//  DYJAlreadyUsedTableViewController.m
//  SEMCRM
//
...
#import "DYJAlreadyUsedTableViewController.h"
#import "DYJAvailableCell.h"
@interface DYJAlreadyUsedTableViewController ()
{
    UILabel *titleView;
...
@implementation DYJAlreadyUsedTableViewController
...
-(void)getData{
    ...
    NSDictionary *dic=@{@"SECNET_CODE":[MyUtil getUserInfo].employee_no,@"STATUS":@"3"};
    [HttpManageTool selectdyzList:dic success:^(NSArray *dyzArr) {
        [dataArr removeAllObjects];
        [dataArr addObjectsFromArray:dyzArr];
        titleView.text = [NSString stringWithFormat:@"已使用抵用卷(%ld)",dataArr.count];
